import { describe, it, expect } from 'vitest'
import { render, screen } from '@testing-library/react'
import ReviewCard from '../components/ReviewCard'

describe('ReviewCard', () => {
  it('renders the review body', () => {
    render(
      <ReviewCard
        rating={4}
        body="Great building."
        date="2026-04-02"
        author="James"
      />
    )
    expect(screen.getByText('Great building.')).toBeInTheDocument()
  })

  it('renders the author name', () => {
    render(
      <ReviewCard rating={5} body="Loved it" date="2026-01-01" author="Priya" />
    )
    expect(screen.getByText('Priya')).toBeInTheDocument()
  })

  it('renders the correct number of filled stars', () => {
    render(<ReviewCard rating={3} body="OK" date="2026-01-01" author="A" />)
    const filled = screen.getAllByText('★')
    expect(filled.length).toBe(3)
  })

  it('shows a delete button only when onDelete is provided', () => {
    const { rerender } = render(
      <ReviewCard rating={3} body="OK" date="2026-01-01" author="A" />
    )
    expect(screen.queryByText('Delete')).not.toBeInTheDocument()

    rerender(
      <ReviewCard rating={3} body="OK" date="2026-01-01" author="A" onDelete={() => {}} />
    )
    expect(screen.getByText('Delete')).toBeInTheDocument()
  })
})
